...
export const ChatScreen: React.FC<ChatScreenProps> = ({
  ...
  // Tracks a newly-created conversation that hasn't been persisted to Firestore yet.
  // Cleared once the first message is sent.
  ...
  // Configure FirestoreServices with userInfo, prefix, blacklist, and storageProvider.
  ...
  useEffect(() => {
    ...
    service.regexBlacklist = blackListRegex ?? undefined;
    if (storageProvider) {
      service.setStorageProvider(storageProvider);
    }
  ...
  // Keep FirestoreServices.conversationId in sync with the UI-selected conversation so
  ...
  useEffect(() => {
    ...
    if (convId) {
      ...
      service.setConversationInfo(convId, others, partnersForService);
    } else {
  ...
  useEffect(() => {
    ...
    const unsubscribe = service.subscribeToListConversation((items) => {
      const webItems = items as unknown as ConversationProps[];
      rawConversationsRef.current = webItems;
      setConversations(webItems);
      // Auto-select first conversation only once
      if (!hasAutoSelectedRef.current && items.length > 0) {
        hasAutoSelectedRef.current = true;
        const first = items[0] as ConversationProps;
        setSelectedConversationId(first?.id);
        // name field is only for group chats; 1:1 display comes from selectedPartners
        setSelectedName(
          (first?.members ?? []).length > 2 ? first?.name || '' : ''
        );
        setSelectedPartners(
          (first?.members ?? [])
  ...
  const handleSendMessage = useCallback(
    async (text: string) => {
      try {
        // Lazy conversation creation: create the Firestore doc on the first send
        if (pendingConversationRef.current) {
          ...
          // Seed names for both users so each side sees the partner's name immediately,
          // without waiting for setConversationInfo to run on the partner's device.
          const names: Record<string, string> = {};
          if (currentUser.name)
            names[String(currentUser.id)] = currentUser.name;
          const partner = selectedPartners[0];
          if (partner?.name) names[partner.id] = partner.name;
          await svc.createConversation(
            pendingId,
            partnerIds,
            undefined, // name is only for group conversations
            partner?.avatar,
            names
          );
        }
    ...
    async (files: File[]) => {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          // Upload via storageProvider, then send the download URL as path
          let downloadURL = '';
          if (storageProvider) {
            const objectUrl = URL.createObjectURL(file);
            const remotePath = `${convId}/${Date.now()}.${ext}`;
            try {
          ...
          await service.sendMessage(
            {
              id: '',
              text: '',
              type: type as MessageProps['type'],
              path: downloadURL,
              extension: ext,
            ...
            i === 0
              ? (replyToAttach as import('@saigontechnology/firebase-chat-shared').ReplyToMessage)
              : undefined
          );
        } catch (err) {
  ...
  const handleSelectConversation = useCallback(
    (conversation: ConversationProps) => {
      ...
      // name field is only for group chats; 1:1 display comes from resolvePartners
      setSelectedName(
        (conversation.members ?? []).length > 2 ? conversation.name || '' : ''
      );
